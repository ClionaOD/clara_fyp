   ...
   for SESSDIR in ${SUBJDIR}/ses-*/; do
	...
	then
		...
		if [[ -f "${DWIPTH}/${SUBJ}/${MYSESS}/dwi.bedpostX/merged_f1samples.nii.gz" ]]; then
			...
			# TODO: double check paths and filenames
			if [[ -f "${DWIPTH}/${SUBJ}/${MYSESS}/probtrackx2_clara/done_run_preparerois" ]]; then
				echo "Already run prepare ROIS"
			else
			# TODO: change this to the correct path
				sbatch ${BASEDIR}/tractography_prepare/run_preparerois.sh $SUBJ $MYSESS
			fi		
		fi
